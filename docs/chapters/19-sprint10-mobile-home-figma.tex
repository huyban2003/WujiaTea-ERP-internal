...
\section{Follow-up (2026-06-23) --- fix lề kép, về đúng 16px BA}

User phản hồi cả trang home mobile bị thụt mép quá (dãn), trong khi BA spec
\(=16\)px. Lần chỉnh đầu (\texttt{.wujia-mhome} \texttt{16→10px}) vẫn dãn ---
\textbf{root cause}: đo box-model browser thấy \texttt{.content-wrapper} của Vuexy
có \texttt{padding ngang \(\approx\)16.8px}, \emph{cộng dồn} với padding của
\texttt{.wujia-mhome} → lề kép (\(\approx\)27--33px) thay vì 16px.

Fix: (1) trả \texttt{.wujia-mhome} về \texttt{padding:16px} (= BA, đúng token
\texttt{-{}-wujia-mshell-content-pad-x}); (2) thêm class \texttt{wujia-home-wrapper}
vào content-wrapper của \texttt{portal\_home.xml} + CSS scoped
\texttt{@media (max-width:991.98px) html body .content .content-wrapper.wujia-home-wrapper \{padding-left/right:0\}}
→ bỏ lề Vuexy CHỈ ở home mobile (\(<\)992px), desktop + trang khác giữ nguyên.
\texttt{.wujia-mhome} thành inset DUY NHẤT \(=16\)px.

Files: \texttt{portal\_home.xml} (+class) + \texttt{\_components.css} (\texttt{.wujia-mhome}
16px + media override). Bump \texttt{\_components.css ?v=1136}, manifest base
\texttt{19.0.5.10.0} + layout \texttt{19.0.24.0.0}. Verify: upgrade RC=0; browser
390px hero \texttt{left=16px} (trước 27px), cả trang (hero + khung giờ + hành động
nhanh) thẳng hàng tại 16px, không scroll ngang; desktop intact (override chỉ
\(<\)992px).
