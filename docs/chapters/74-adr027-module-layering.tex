...
\item \texttt{hooks.py} + \texttt{'pre\_init\_hook'} \textbf{khai trong manifest và import ở
  ...
  \texttt{ec6d380}, 16/09), nên chép file thôi thì hook không chạy. Hàm
  \texttt{migrate\_ownership(cr, old, new, names, models)} đổi chủ ba chỗ:
  \begin{itemize}
  \item \texttt{ir\_model\_data}: \texttt{names=None} chuyển \emph{tất cả} (module cũ bị rút hết,
    như F7); tách một phần (F8+) thì truyền danh sách. Đừng quên xmlid của field trên model
    \emph{kế thừa} (F7 có 7: \texttt{res.config.settings}, \texttt{sale.order}, kể cả
    \texttt{display\_name}/\texttt{id}) và \texttt{model\_<model kế thừa>}.
...
\item Module cũ: còn controller/QWeb/asset thì giữ, thêm depends \texttt{wujia\_<x>}. Rút hết
...
\item \textbf{Gỡ vỏ} (khi module cũ rút hết): đo trên DB copy trước --- \texttt{button\_immediate\_uninstall}
  ...
  Uninstall một cú trên Apps; xoá thư mục khỏi repo ở phiên review (FR-P) khi mọi DB đã gỡ.
\end{enumerate}
...
\texttt{number\_next} của sequence. Kèm: test cũ xanh y hệt; DB trắng chỉ cài \texttt{wujia\_<x>}
