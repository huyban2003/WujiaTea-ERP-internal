...
\textbf{F10 (26/09) --- tách một phần \texttt{support}:} 2 model, 4 menu, 7 danh mục mẫu, sequence
\texttt{WJ-TK/} sang \texttt{wujia\_support} (depend \texttt{sale}, \texttt{stock\_picking\_batch},
\texttt{sales\_team}); không có nhóm quyền riêng. Hook đổi chủ 113 xmlid + 16 ràng buộc + 1 quan hệ, portal
còn 3 QWeb + 2 mục nav; snapshot 130 dòng đổi chủ, lệch duy nhất là \texttt{write\_date} của bảng danh mục
(nạp lại noupdate, giá trị giữ). Controller mỏng: phạm vi (theo \emph{người tạo}), ``kiểm + tạo + đính kèm''
trong một savepoint thay cho \texttt{unlink}, trả lời ticket, ``đính kèm thuộc ticket'' về model
(\texttt{\_portal\_scope\_domain}, \texttt{create\_from\_portal} trả mã lỗi của form,
\texttt{\_portal\_reply}, \texttt{\_portal\_get\_attachment}); bảng badge ticket mobile rời
\texttt{portal\_base} về màn sở hữu (A2). HTML 18/18 GET giống từng byte, 9/9 POST cùng redirect, DB sau POST
giống hệt; suite 872/0; mutation 4/4 luật đỏ.

\textbf{F11 (26/09) --- tách một phần \texttt{notification}:} 3 model (giữ \texttt{\_name}), 2 nhóm quyền
+ privilege, 2 rule theo cửa hàng, sequence \texttt{ANN/}, 5 loại mẫu, 4 menu sang \texttt{wujia\_notification}.
Nhóm quyền không có trong \texttt{ir\_model\_data} theo model nên phải ghi vào \texttt{extra} của hook; tham chiếu
\texttt{group\_*} chỉ có trong chính module (ACL, menu) nên đổi thành ref cục bộ. Hook đổi chủ 140 xmlid + 28 ràng buộc
+ 5 quan hệ, portal còn 4 QWeb + chuông + mục nav; snapshot 173 dòng đổi chủ, lệch duy nhất là màu nền loại
(bẫy noupdate, bước 2), người trong nhóm và \texttt{last\_value} sequence giữ. Controller mỏng (433 $\rightarrow$ 336
dòng): domain lịch sử/còn hiệu lực, đếm chưa đọc theo cửa hàng, đính kèm thuộc thông báo về model; 3 chỗ ghi
``đã đọc'' thành một \texttt{wujia.notification.read.\_mark\_read}. Home gọi chung luật (như F9): KPI chưa đọc
bằng badge chuông, list bỏ bài hẹn giờ/hết hạn. HTML 117/120 giống từng byte (3 lệch đều Home, đúng chỗ vá),
DB đã đọc sau lượt ghi giống hệt; suite 885/0; mutation 5/5 luật đỏ.

\textbf{F12 (26/09) --- tách một phần \texttt{exam}, gộp F12a + F12b:} 5 model (giữ \texttt{\_name}), 2 nhóm
quyền + privilege, 2 rule, 3 sequence, 7 menu sang \texttt{wujia\_exam}; mobile anh Thái
(\texttt{wujia\_mobile\_portal\_exam}) đổi depend + 18 dòng ref như F8 --- gỡ vi phạm R3 cuối, \texttt{check\_layers}
về \textbf{0}. Hook đổi chủ 214 xmlid + 30 ràng buộc + 1 quan hệ, portal còn 3 QWeb + 2 mục nav; snapshot 245 dòng
đổi chủ, bản ghi + \texttt{last\_value} 3 sequence + người trong nhóm giữ. Lộ bẫy mới: \texttt{groups=} trong
\emph{arch} view phải ghi đủ \texttt{module.xmlid} (Odoo 19 bỏ qua im lặng tên không khớp $\Rightarrow$ 10 nút
backend ẩn với cả Administrator); ACL csv và \texttt{menuitem} thì ref cục bộ được. Controller mỏng (729
$\rightarrow$ 558 dòng): luật chọn ca, lịch tháng, meta khoá, phạm vi cửa hàng và
\texttt{register\_from\_portal} (lớp lỗi \texttt{ExamPortalError} giữ mã \texttt{not\_found}/\texttt{validation}/
\texttt{business}) về model; ``tối đa người/phiếu'' thành một nguồn (ca thi, trống thì khoá) cho cả hướng dẫn,
chặn portal và constraint. HTML/JSON 234/234 giống từng byte (19 nhánh gửi phiếu), DB sau ghi giống hệt; suite
897/0; mutation 6/6 luật đỏ.

\textbf{F13 (26/09) --- tách một phần \texttt{return}, gộp F13a + F13b, phân hệ cuối khối A:} 6 model (giữ
\texttt{\_name}, gồm kế thừa \texttt{sale.order}/\texttt{stock.picking}/\texttt{product.product}), wizard xử lý bù,
2 nhóm quyền + privilege, rule, 2 sequence, 5 loại lỗi, 6 menu sang \texttt{wujia\_return} (230 xmlid + 54 ràng buộc
+ 3 quan hệ); portal còn 3 QWeb + 2 mục nav; không module mobile nào ref $\Rightarrow$ không đụng code anh Thái.
\texttt{groups=} trong arch ghi đủ tiền tố ngay từ đầu. Snapshot 287 dòng đổi chủ, bản ghi + \texttt{last\_value}
2 sequence + người trong nhóm giữ. Controller mỏng (588 $\rightarrow$ 357 dòng): đơn hợp lệ 10 ngày, phạm vi cửa
hàng, cấu hình bù, minh chứng (controller chỉ sniff MIME + đo dung lượng), \texttt{create\_from\_portal} trong
savepoint (thay \texttt{unlink} thủ công), khoá trạng thái/lọc và tiến độ bù về model. A2: một bảng nhãn
\texttt{RETURN\_STATUS\_LABELS} ở \texttt{portal\_base} cho Home PC + mobile + \texttt{/portal/return}; Home gọi
\texttt{\_portal\_open\_domain}/\texttt{\_portal\_recent\_domain} qua \texttt{hasattr}. HTML 298/306 giống từng byte, 8
khác đều là nhãn Home mobile đổi có chủ đích (``Chờ xử lý'' $\rightarrow$ ``Đã gửi''\ldots, màu giữ); DB sau ghi
giống hệt; suite 912/0; mutation 7/7 luật đỏ. \texttt{PENDING\_SPLIT} rỗng $\Rightarrow$ mở $\star$FR-A.

\textbf{$\star$FR-A (26/09) --- review toàn khối A, ĐẠT, khối A khép} (\texttt{docs/f-review-FR-A.md}): HEAD
\texttt{164d8ec} đo cạnh đối chứng \texttt{9d2a606} cùng seed. Tầng: \texttt{check\_layers} 0 vi phạm phía Dev (R7 còn 2
của anh Thái); 7 module L2 không controller/static/\texttt{http}/xmlid portal trong \texttt{.po}; L3b chỉ giữ 2 ngoại lệ đã
chốt (\texttt{wujia.portal.cart}, AbstractModel \texttt{wujia.portal.debt}); 7 chỗ \texttt{portal\_base} gọi L2 đều qua
\texttt{hasattr}. Hiệu năng: số query 17 route $\times$ 4 user giống hệt đối chứng; index đủ trên 6 bảng chính. Sạch: xoá
4 hàm chết, \texttt{is\_read\_by}, \texttt{ormcache}/import thừa, bẫy \texttt{number\_next} sequence \texttt{INF/}, 14 comment
mã phiên (11 file, +16 $-$112); suite 914/0. Mutation M7 (bỏ \texttt{extra} của hook) không đỏ vì \texttt{pre\_init} chỉ chạy
lúc cài $\Rightarrow$ \texttt{split\_snapshot} là chốt duy nhất, ghi vào bước 4 quy trình. Nợ chỉ ghi: \texttt{portal\_sale}
còn write ở update/remove/note; \texttt{\_sql\_constraints} 16 file (8 phía Dev); \texttt{auto\_install} ý \#3 chưa áp;
7 câu hỏi BA còn treo.
