\chapter{ADR-027 --- Kiến trúc tầng module \& Mobile core (đề xuất 2026-09-17, chốt 2026-09-26)}
\input{chapters/74-adr027-figures}
\sloppy

\emph{\textbf{Trạng thái: ĐÃ CHỐT 26/09/2026, đã áp dụng cho khối A} (F7--F13 tách 7 phân hệ, $\star$FR-A
review đạt --- chapter Sprint 63). Ba quyết định bổ sung chốt cùng ngày ở \S\ref{sec:adr27-addendum};
riêng \texttt{auto\_install} mới ghi, chưa áp code.}

\emph{ADR này trả lời câu hỏi của BA và chủ dự án: muốn dựng \emph{mobile core} cho backend ERP thì
module phải phụ thuộc nhau thế nào, để sau này tách Wujia thành các folder/repo riêng như
beesuite mà không bị kéo ngược lên portal. Số đo hiện trạng trong chapter đo trên \texttt{main}
\texttt{12750a2} (sau khi pull 109 commit) và trên UAT chỉ-đọc qua XML-RPC; kết quả áp dụng ở chapter Sprint 63.}
...
\item \textbf{L2} gồm hai nhóm \textbf{cùng cấp}:
  \begin{itemize}
  \item nghiệp vụ: 8 module đang sạch + 7 phân hệ tách ra (\texttt{wujia\_order\_window}, \texttt{\_info\_request}, \texttt{\_knowledge}, \texttt{\_support}, \texttt{\_notification}, \texttt{\_exam}, \texttt{\_return});
...
\section{Bổ sung khi chốt (26/09) --- chia portal, kế thừa view mobile, \texttt{auto\_install}}
\label{sec:adr27-addendum}

Ba ý dưới đây đã bàn với BA, anh Thái và chủ dự án từ 17/09 và được chốt vào ADR khi khép khối A.

\subsection*{1. Portal chia theo chức năng, mobile chia theo module backend}

BA chia portal theo \textbf{chức năng người dùng thấy}, không theo module backend, và code đang đúng như vậy:
\texttt{portal\_report} và \texttt{portal\_purchase\_history} không có module nghiệp vụ cùng tên;
\texttt{portal\_sale} depend \texttt{wujia\_sale} cùng hai module portal khác. Vì vậy ký hiệu
\texttt{wujia\_portal\_<x>} được hiểu là \texttt{portal\_}<chức năng>: \textbf{một chức năng depend 1..N module nghiệp vụ,
không cần 1--1}. Mobile là menu ERP thu nhỏ nên chia theo module backend
(\texttt{mobile\_<module>}). Hai kênh chia khác nhau là đúng. Bảy phân hệ tách ở khối A không bị ảnh hưởng.

\subsection*{2. Mobile kế thừa view thế nào}

BA nêu: PC ít dùng kanban, mobile dùng form/list, và view \texttt{sale.order} đã bị nhiều module đè. Luật:
\begin{itemize}[nosep]
\item \textbf{CSS áp lên DOM đã render} nên không cần priority hay xpath. Thứ tự nạp theo depends: ví dụ
  \texttt{mobile\_sale\_stock} depend \texttt{mobile\_sale} $+$ \texttt{sale\_stock}, \texttt{auto\_install}.
  File nạp sau thắng khi cùng selector.
\item \textbf{Cấm view \texttt{mode="primary"} priority cao cho mobile}, vì sẽ mất field mà các module khác đã
  inherit vào view gốc. Chỉ dùng extension $+$ xpath vào view gốc. \textbf{Cấm \texttt{position="replace"}}.
  Muốn ẩn/hiện thì dùng \texttt{<attribute name="class" add="d-none d-md-block" separator=" "/>}. Priority cao
  (100) chỉ dùng khi thật sự cần áp cuối cùng.
\item \textbf{Bậc làm}, dừng ở bậc thấp nhất đủ dùng: (1) CSS chung ở \texttt{mobile\_core} $\to$ (2) CSS riêng màn,
  xpath gắn một class vào \texttt{<form>} $\to$ (3) xpath \texttt{attributes} ẩn/hiện $\to$ (4) kanban riêng mobile,
  càng ít càng tốt.
\item \textbf{Chưa đo:} Odoo 19 $+$ MuK có tự chọn kanban thay list trên màn nhỏ không. Phải đo trên UAT giả lập
  điện thoại trước module mobile đầu tiên.
\end{itemize}

\subsection*{3. \texttt{auto\_install} cho module ghép}

\begin{itemize}[nosep]
\item Module ghép L3b (\texttt{portal\_}<chức năng>, \texttt{mobile\_<x>}) là cầu nối kênh $\leftrightarrow$
  nghiệp vụ, nên được \texttt{auto\_install: True}, nhưng \textbf{chỉ khi là module ghép thuần, không có model}:
  \texttt{portal\_report} (sale $+$ base), \texttt{portal\_purchase\_history} (sale $+$ delivery $+$ base),
  \texttt{portal\_delivery} (delivery $+$ base), và 3 module \texttt{mobile\_*} của anh Thái.
\item Module còn model thì giữ \texttt{False}: \texttt{portal\_sale} (giỏ hàng) và \texttt{portal\_debt}
  (AbstractModel seam; soi lại khi áp). Bảy \texttt{portal\_<x>} sau khối A đã hết model nghiệp vụ, nên xét lại
  cùng đợt áp.
\item \texttt{wujia\_portal\_base} (L3a) và \texttt{wujia\_mobile\_core} giữ \texttt{False}: đây là công tắc bật
  kênh, phải cài chủ động.
\item \textbf{Áp đồng loạt trong một phiên}, không lẻ từng module. Đổi cờ không có tác dụng trên DB đã cài (UAT),
  chỉ có tác dụng trên DB mới và DB test. \textbf{Tới 26/09 mới ghi, chưa áp code.}
\end{itemize}
...
\item \textbf{Depends của phân hệ tách ra suy thẳng từ quan hệ} (manifest thật sau F13):
  \texttt{wujia\_return} $\rightarrow$ \texttt{wujia\_sale} $+$ \texttt{wujia\_franchise} (\texttt{sale.order});
  \texttt{wujia\_support} $\rightarrow$ \texttt{wujia\_franchise} $+$ \texttt{sale},
  \texttt{stock\_picking\_batch}, \texttt{sales\_team}; \texttt{wujia\_order\_window} $\rightarrow$
  \texttt{wujia\_sale}; \texttt{wujia\_exam}, \texttt{wujia\_notification},
  \texttt{wujia\_info\_request}, \texttt{wujia\_knowledge} $\rightarrow$ \texttt{wujia\_franchise}
  (\texttt{knowledge} không có Many2one sang Wujia, depend vì quyền theo cửa hàng).
\end{enumerate}
...
\item Tạo \texttt{wujia\_<x>} bằng \texttt{git mv}: \texttt{models/}, view backend, menu,
  ...
  Dữ liệu mẫu cũng vậy (F10, 7 danh mục ticket): đo trên DB giả lập, field ghi trong XML
  (\texttt{sequence}) bị đưa về giá trị XML, còn bản dịch vi\_VN tuỳ biến \emph{giữ nguyên} (\texttt{.po}
  không đè bản dịch đã có); snapshot chỉ thấy \texttt{write\_date} đổi.
  F11 là lần đầu lệch thật: UAT còn 4 màu nền loại thông báo cũ (Sprint 4.3) khác XML (Sprint 19);
  \texttt{-i} đưa về màu XML, chủ dự án chốt theo code.
...
\item Module cũ: còn controller/QWeb/asset thì giữ, thay depends nghiệp vụ bằng
  ...
  phía thì ghi ở cả hai file). F8: 137 mục thành 56 portal + 81 nghiệp vụ; F9: 123 thành 26 + 97; F10: 204 thành 75 + 129; F11: 214 thành 55 + 159; F12: 393 thành 177 + 216; F13: 333 thành 94 + 239. Rút hết
...
\item \textbf{Ref nhóm quyền đổi chủ (F11, F12):} ACL csv và \texttt{<menuitem groups=\ldots>} đổi sang tên
  cục bộ được (loader tự thêm module đang nạp). \textbf{\texttt{groups=} trong arch view thì phải ghi đủ
  \texttt{wujia\_<x>.group\_\ldots}}: Odoo 19 parse bằng \texttt{\_get\_group\_definitions().parse(\ldots,
  raise\_if\_not\_found=False)}, tên thiếu tiền tố không khớp nhóm nào và bị bỏ qua im lặng $\Rightarrow$ phần tử
  ẩn với mọi người, kể cả Administrator (F12: 10 nút trên 3 form). Snapshot bắt qua md5 arch.
...
\textbf{F8--F13 và $\star$FR-A (26/09):} sáu phân hệ còn lại tách theo đúng quy trình trên, mỗi phiên một
lần deploy UAT, 0 lệch thật ngoài các dòng đã giải trình; $\star$FR-A review toàn khối đạt. Số đo từng phiên, bảng
module/luật dời về model và nợ còn lại ở chapter Sprint 63. Hai bài học quy trình rút ra:
\begin{itemize}[nosep]
\item \textbf{Test rollback không viết bằng \texttt{assertRaises}}: Odoo tự bọc savepoint, nên test vẫn Pass
  khi code quên savepoint (F8, bắt qua mutation). Viết bằng \texttt{try/except} thường.
\item \textbf{Hook quên \texttt{extra} chỉ snapshot bắt được}: \texttt{pre\_init\_hook} chỉ chạy lúc cài, trạng thái
  cuối giống hệt, chỉ khác id menu (F9; FR-A mutation M7). \texttt{split\_snapshot} trước/sau là chốt duy nhất,
  không thay được bằng test đơn vị.
\end{itemize}
...
P5 & \texttt{support} & \textbf{Xong F10 (26/09)}: tách một phần, 113 xmlid + 16 ràng buộc + 1 quan hệ đổi chủ, sequence + 7 danh mục giữ; bảng badge ticket rời \texttt{portal\_base}. \\
P6 & \texttt{announcement} & \textbf{Xong F11 (26/09)}: tách một phần sang \texttt{wujia\_notification}, giữ \texttt{\_name}; 140 xmlid + 28 ràng buộc + 5 quan hệ đổi chủ, 2 nhóm quyền + sequence ANN giữ; Home dùng chung luật chưa đọc. \\
P7 & \texttt{exam} & \textbf{Xong F12 (26/09)}: tách một phần sang \texttt{wujia\_exam}, giữ \texttt{\_name}; 214 xmlid + 30 ràng buộc + 1 quan hệ đổi chủ, 2 nhóm quyền + 3 sequence giữ; mobile Thái đổi ref, \texttt{check\_layers} về 0. \\
P8 & \texttt{return} & \textbf{Xong F13 (26/09)}: tách một phần sang \texttt{wujia\_return}, giữ \texttt{\_name}; 230 xmlid + 54 ràng buộc + 3 quan hệ, 2 nhóm quyền + 2 sequence giữ; một bảng nhãn trạng thái cho Home + portal; \texttt{PENDING\_SPLIT} rỗng. \\
\bottomrule
\end{tabularx}
\end{center}
...
\item \textbf{Tên module phân hệ thông báo}: đề xuất ban đầu là \texttt{wujia\_announcement}, vì BA đã
  dành tên \texttt{notification\_core} cho nền tảng gửi thông báo (sự kiện + kênh). \textbf{Chốt ở F11:
  giữ \texttt{wujia\_notification}} (không trùng \texttt{notification\_core}); \texttt{\_name} vẫn là
  \texttt{wujia.notification}.
\end{enumerate}
...
\item \textbf{Module \texttt{wujia\_portal\_<x>} còn được chứa phần duyệt/xử lý nghiệp vụ không?}
  ...
  \textbf{Chủ dự án đã cho làm theo ADR (F13, 26/09)}; BA chỉ cần xác nhận lại để sửa bảng.
...
\textbf{Sau khi áp (26/09):} lộ trình P2--P8 đã chạy xong. Bảy phân hệ là module nghiệp vụ L2, và
\texttt{check\_layers} báo 0 vi phạm phía Dev. Người dùng vẫn không thấy thay đổi nào, ngoài vài lỗi lệch luật giữa
Home và màn chi tiết được vá kèm. P1 (\texttt{mobile\_core}) và 7 câu hỏi trên vẫn chờ BA.
