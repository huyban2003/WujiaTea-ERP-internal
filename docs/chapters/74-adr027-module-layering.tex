...
\textbf{Các bước} (khuôn \texttt{wujia\_franchise\_inspection/hooks.py}):
\begin{enumerate}
\item Tạo \texttt{wujia\_<x>}: chuyển \texttt{models/}, view backend, menu, \texttt{security/},
  \texttt{data/} (sequence, cron), test model, và phần \texttt{i18n/vi\_VN.po} tương ứng.
\item \textbf{Model dính chặt nhau đi cùng một module} (One2many $\leftrightarrow$ Many2one hai
  ...
  học ADR-015).
\item \texttt{wujia\_portal\_<x>} giữ controller, QWeb, asset; thêm depends
  \texttt{wujia\_<x>}; bump version.
\item \texttt{pre\_init\_hook} của \texttt{wujia\_<x>} đổi cột \texttt{module} cho danh sách xmlid
  đã chuyển và cho \texttt{model\_*}/\texttt{field\_*}/\texttt{constraint\_*}. Chạy ở
  \texttt{pre\_init\_hook} vì Odoo \emph{không} chạy migration script ở lần cài đầu.
\item Sửa tham chiếu \texttt{wujia\_portal\_<x>.group\_\ldots} ở module khác (đo được 0--4 file
  mỗi phân hệ, phần lớn là test của \texttt{wujia\_portal\_layout}).
\item Deploy một lệnh: \texttt{-i wujia\_<x> -u wujia\_portal\_<x>}.
\end{enumerate}

\textbf{Kiểm bằng số, trước và sau, trên DB copy cô lập} (không đụng \texttt{wujia\_tea\_19}):
số bản ghi từng model; số user trong từng nhóm quyền; \texttt{number\_next} của sequence; số
menu/action/rule; \texttt{ir\_model\_data} của module cũ chỉ còn phần kênh. Mọi con số phải khớp
tuyệt đối. Test cũ xanh y hệt, có run đối chứng (bài học Sprint 57).
...
P2 & Tách \texttt{order\_window} (pilot) & 7 xmlid, 0 nhóm quyền, 0 tham chiếu ngoài. Thực chất chỉ bỏ chữ ``portal''. Quy trình mà có lỗi thì lộ ở đây với giá rẻ nhất. \\
P3 & \texttt{info\_request} & 13 xmlid, 0 nhóm quyền. \\
P4 & \texttt{knowledge} & 20 xmlid, có 1 cron. \\
