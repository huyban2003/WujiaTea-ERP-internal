...
| F10 | Tách một phần `support` → `wujia_support` + controller mỏng (kèm trả lời ticket); bảng badge ticket rời `portal_base` | support, portal_support, portal_base | Cao | ✅ 26/09 — `docs/f10-acceptance-matrix.md` (9/9); snapshot 130 đổi chủ, lệch duy nhất `write_date` danh mục; `033794c`, chờ deploy UAT |
