...
| F12 | Tách một phần `exam` → `wujia_exam` (giữ `_name`, 2 nhóm quyền, 3 sequence) + controller mỏng (`register_from_portal`, max/phiếu một nguồn cả constraint); mobile Thái đổi ref | exam, portal_exam, mobile_portal_exam | Cao | ✅ 26/09 — `docs/f12-acceptance-matrix.md` (9/9); snapshot 245 đổi chủ, HTML 234/234; `check_layers` 0; `2acbd9f`, chờ deploy UAT, báo Thái trước |
| F13 | Tách phân hệ + controller mỏng (**bỏ `number_next` khỏi XML sequence trước khi tách**; đo dữ liệu mẫu noupdate trên UAT so XML; **`groups=` trong arch view ghi đủ `wujia_<x>.group_…`** — bài học F12) | theo phân hệ | Cao | ☐ |
