...
| F12 | Tách một phần `exam` → `wujia_exam` (giữ `_name`, 2 nhóm quyền, 3 sequence) + controller mỏng (`register_from_portal`, max/phiếu một nguồn cả constraint); mobile Thái đổi ref | exam, portal_exam, mobile_portal_exam | Cao | ✅ 26/09 — `docs/f12-acceptance-matrix.md` (9/9); snapshot 245 đổi chủ, HTML 234/234; `check_layers` 0; chờ deploy UAT, báo Thái trước |
