...
| **D4e** | `wj-rep-mcard` (**3**) + `wj-pc-metric-card` của trang báo cáo (**4**) | **7 / 1** | `wujia_portal_report`, `wujia_portal_layout` | Trung bình — `wj-rep-mcard` **không có viền hẳn**, thêm viền là đổi hình học 3 khối báo cáo | ✅ **Chặn đã gỡ 05/09**: `/portal/reports/orders` trả **200** với tz `Asia/Saigon` trên DB mới (xem inventory §13.3) ⇒ đo được trên local. Con số 36 cũ là grep thô, xem **inventory §13.1** |
