...
- **S37 — mobile issue-list + page-header DONE (2026-07-16):** 7 issue UI (Home spacing/typography/clamp + Đặt hàng name/thumbnail/floatbar/search) + 4 page-header về `wj_page_header`. UI-only, không schema. **Deploy**: auto-deploy đủ (bump layout 31.3.0 / sale 4.3.0 / base 5.13.0 / report 1.5.0, `?v=1154`), **KHÔNG cần `-i`**. ⚠️ Pending: (a) các issue "Need Confirm: Yes" — user uỷ quyền default (section 18px, clamp 2 dòng, spacing 16px), BA có thể chốt token khác; (b) report mobile top-spacing hơi rộng hơn các trang `.wujia-mpage` (report là single-layout non-mpage — chấp nhận); (c) UI-MOB-ORDER-002 thumbnail dùng `image_128` — SP chưa có ảnh sẽ hiện icon-package placeholder (đúng ý BA).
