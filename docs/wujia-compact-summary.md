...
- **M — portal Đăng ký thi**: ✅ **wire xong ở Sprint 45 (ch.58)** — XEM (list/chi tiết/kết quả/khóa/lịch/khung giờ) dữ liệu thật CẢ mobile+PC, submit thật MOBILE; legacy `schedule`/`result` đã gỡ hẳn; `19.0.5.0.0` (migration drop bảng + dọn phiếu mồ côi). **Còn tồn: PC submit (defer S46 — nối 3 hàm mock `portal_exam_pc.js` vào 3 endpoint đã có `/calendar`,`/slots`,`/register`; prompt sẵn trong plan `ethereal-snacking-hare.md`, task set Done khi xong)**; deploy `-u wujia_portal_exam` (có migration, KHÔNG `-i`). CHƯA DEPLOY UAT.
