...
**Cập nhật:** 2026-06-22 — **Sprint 21 DONE.** S21 = chuẩn hoá **v2** 2 cụm mobile theo BA Figma copy (tinh chỉnh kích thước component + dùng canonical `wj-*` S19): **Product/Cart v2** (`4575:777` → `wujia_portal_sale` catalog+cart: chips danh mục `wj-filter-chip` thay `<select>`, card-per-row, qty badge green→**cyan**, **warnbar khung giờ wire vào giỏ** (BE read-only nhỏ), summary sticky restyle) + **Order History v2** (`4577:1112` → `wujia_portal_purchase_history` list+detail: card-per-row + badge đổi màu theo state, search box icon+placeholder, **back-button detail**). Desktop giữ nguyên, UI-only. S20 (đổi trả) + S18/19 đã push trước. **▶ NEXT: tiếp UI mobile** — status batch S13, khung giờ cart submit S11, i18n. Chi tiết → §4 + §5 + chapter 30. _(Lịch sử prose sprint cũ đã GỠ để giữ compact.)_
...
→ Chi tiết: `chapters/04-17.tex` + `chapters/18-sprint9-issue-list-ui-refactor.tex` (cuối Sprint 9) + `chapters/19-sprint10-mobile-home-figma.tex` + `chapters/20-sprint11-mobile-order.tex` + `chapters/21-sprint12-mobile-shell-final.tex` + `chapters/22-sprint13-mobile-order-history.tex` + `chapters/23-sprint14-mobile-more-sheet.tex` + `chapters/24-sprint15-mobile-knowledge.tex` + `chapters/25-sprint16-mobile-dashboard-3screens.tex` + `chapters/26-sprint17-mobile-ticket.tex` + `chapters/27..29` (S18 account / S19 chuẩn hóa / S20 đổi trả-bù hàng).

---

## §5 wujia-current-status

**State (2026-06-22) — SPRINT 21 DONE; S18/19/20 đã push trước.** 18 module active.
...
- **▶ NEXT SESSION: tiếp UI mobile.** Đổi trả/Bù hàng ✅ (S20). Pending còn: status giao hàng từ batch state thật (S13); khung giờ vào cart (S11); i18n `.po`; wire `uom_id` + field bù hàng Phase 2 (Đã nhận/ngày mở-sản xuất/video/response HQ).
