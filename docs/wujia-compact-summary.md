...
**Cập nhật:** 2026-07-14 — **Sprint 33 DONE: WJ_PageHeader — chuẩn hoá header mọi trang portal (Mobile + PC) theo BA spec — chapter 44.** Component QWeb MỚI `wujia_portal_layout.wj_page_header` (3 variant title/back/create, props qua t-call `ph_variant/ph_title/ph_back_url/ph_create_url/ph_can_create/ph_platform`; visibility `d-flex d-lg-none` / `d-none d-lg-flex` bake sẵn; modifier `--m`/`--pc`) + CSS `.wj-page-header*` (base+`--m` ở `_components.css`, `--pc` ở `_pc_components.css`, token `--wj-*` sẵn; mobile title 22/28 w800 · back 40 (tap 44) · create 112 · spacing 16/44/12 · x=16; PC 28/36 w800 · back 44 · create 122 · gap-back-title 18 · spacing 24/44/20). Breakpoint dùng **991.98** (project split) qua modifier class, KHÔNG media-query 767 của spec. Rollout ~40 header-site/11 module: **reconcile 4 trang PC cũ** (history/notification/delivery/order — bỏ `__crumb`, 26/700→28/800, back text-phải→**icon-trái 44**); **header-swap 6 trang PC legacy** (return/support/knowledge/exam/report/info-request — chỉ header row, thân desktop giữ legacy); **mobile thay hết bespoke** `wujia-mhist/mnoti/mticket/mknow/mdelivery/mcart/morder/mexam-*title/-back`; single-layout (info-request/report/product-detail) gọi cả 2 header. **GIỮ đặc thù**: order catalog warning-bar/floatbar, cart realtime fragment (JS chỉ phụ thuộc class stepper/row/del, không header), exam **wizard step-back** (`wujia-mexam-back/head/*` còn 4-8 usage), Home dashboard. **EXCLUDE**: Tài khoản (shell `pc_account_layout` Sprint 28) + Home. Noti-list nút "Đánh dấu đã đọc" (không phải create) **dời header→card-head** (giữ id/JS); status badge return/support/info-detail **dời header→block dưới**; count-meta/subtitle tách ra line `wj-count-meta` dưới header. **Gotcha #13 tái phát**: global `h1{font-size … !important}` ép title 32px cả 2 nền tảng → fix scope 2 lớp + `!important` cho 22/28 (weight 800 không dính). Verify: upgrade 11 mod RC=0, Playwright font đo 22px mobile/28px PC khớp mockup, shell nguyên vẹn, EXCLUDE 0 `wj-page-header`, test_sprint9 7/0; report có 500 **pre-existing** (`psycopg2: time zone "Asia/Saigon"` — tz data DB local, không liên quan). Deploy **UI-only, không schema/`-i`**: bump 11 manifest + CSS `?v=1149/1141→1151`. **DEFERRED**: gỡ ~30 block CSS bespoke chết (đã grep 0-usage XML, để cleanup riêng vì rủi ro đụng class wizard). _(Trước: Sprint 32 controller Thông báo BA FINAL + toolchain đọc spec BA (chat GPT + xlsm) — chapter 43; Sprint 31 lịch sử đơn hàng + realtime cart — chapter 42; Sprint 30 order controller — chapter 41. Prose chi tiết các sprint cũ GỠ khỏi đầu file → xem §4 bảng + chapters/.)_
...
**State (2026-07-14) — SPRINT 33 DONE: WJ_PageHeader chuẩn hoá header mọi trang portal (Mobile+PC), chapter 44.** 18 module active.
...
- BA spec xlsm `WujiaTea/docs/Wujia_Internal ERP Master Plan.xlsm` (sheet "1. Model Field" backend, "2. FE - Portal" frontend, "3. Controller" bảng CT-0xx, "5. Issue List" UI sprint 9, "FEATURE CHECKLIST").
