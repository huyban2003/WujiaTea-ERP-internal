...
| `wujia_portal_return` | `/portal/return` single-product + duyệt + `wujia.compensation.allocation` + wizard "Xử lý bù hàng" SO 0đ FIFO + hook picking (Sprint K) |
| `wujia_portal_notification` | `/portal/notification` + models |
| `wujia_portal_exam` | `/portal/exam` (demo) + backend Đăng ký thi: time.slot/course/session/registration(.line) + kết quả (Sprint M) |
...
| Sprint | Date | Outcome |
|---|---|---|
| K | 2026-07-17 | **Backend Bù hàng/Return (K1+K2 chung)**: redesign `wujia_portal_return` single-product + duyệt + `wujia.compensation.allocation` + workflow 8 state + migration `19.0.2.0.0` + `sale.order.is_return_order` (K1); wizard "Xử lý bù hàng" 3 TransientModel → 1 SO 0đ/cửa hàng FIFO + hook `stock.picking` compensated-from-picking (K2). Hardening adversarial review (group-key approved_uom / exact rounding DOWN / portal ACL read-only / state re-validate / uom-root guard / index+outgoing-only). ORM 46/46. → chapter 49 |
...
**State (2026-07-18) — SPRINT M DONE: Backend Đăng ký thi (7 model, backend-only), chapter 50; trước đó K portal-wire tiến độ bù, chapter 49.** 18 module active.
- **Sprint M — backend Đăng ký thi DONE (2026-07-18):** `wujia_portal_exam 19.0.2.2.0 → 19.0.3.0.0` (+depends `wujia_franchise,mail`). 4 model MỚI: `wujia.exam.time.slot` (ca giờ) · `wujia.exam.course` (khóa thi, seq CRS/, publish gate) · `wujia.exam.session` (kỳ thi, mail.thread, seq EXS/, compute start/end tz Asia/Ho_Chi_Minh, reserved/available store `_read_group`, workflow open/close/done/cancel + publish kết quả gate) · `wujia.exam.registration.line` (thí sinh free-text, phone regex VN, `fields.Image` auto-resize 1920, result inline). **Rework** `wujia.exam.registration` (bỏ schedule_id, rename user_id→requester_user_id, +session/course/member/line_ids, capacity **FOR UPDATE NOWAIT** + `_check_booking_allowed`, confirm/reject/cancel). Dormant `schedule`/`result` giữ (DEPRECATED, fix related vỡ). Migration `19.0.3.0.0` (drop uniq + rename cột + drop schedule_id). Security `group_exam_user/manager` + ACL portal-read-only + rule own-franchise. Backend house-style views (statusbar/ribbon/chatter/result inline) + menu "Đào tạo / Thi" dưới `menu_wujia_root`. **Portal GIỮ demo** (controller demo-safe, register/cancel JSON = stub deferred). Verify: `-u` RC=0 + migration sạch + ORM (capacity lock/publish gate/validate/cancel-frees) all pass + 12 view get_view + portal 6 route + home + return 200 + test_sprint9 7/7. **Deploy prod**: `-u wujia_portal_exam` (chạy migration; **KHÔNG** cần `-i`). ⚠️ **Pending M**: (a) **portal wire thật** (chọn kỳ thi mở → nhập nhân sự → gửi phiếu, lịch sử + kết quả — hiện demo); (b) deprecate/bỏ hẳn `schedule`/`result`; (c) warning `session_id` NOT-NULL do dòng demo cũ NULL (prod sạch OK). **Nguồn**: chapter 50 + plan `floating-nibbling-widget.md`.
- **Sprint K — backend Bù hàng DONE (2026-07-17):** `wujia_portal_return 19.0.2.0.0` redesign single-product + duyệt + allocation + wizard SO 0đ FIFO + hook picking. **Deploy prod**: `-u wujia_portal_return wujia_sale` (module đã cài → auto-upgrade chạy migration `19.0.2.0.0` drop bảng line cũ + rename cột; **KHÔNG cần `-i`**). ✅ **Portal wire tiến độ bù DONE (2026-07-17, `wujia_portal_return 19.0.2.1.0`, chapter 49 append):** cửa hàng xem `/portal/return/<id>` thấy phương án xử lý + SP/SL duyệt bù + progress bar đã bù/còn thiếu + tình trạng bù + đơn bù 0đ & trạng thái giao + phản hồi duyệt HQ (desktop+mobile, read-only, KHÔNG schema, auto-deploy no `-i`). Nút "Đặt hàng bù" **bỏ** (kiến trúc HQ-driven, cửa hàng không tự đặt); "xác nhận đã nhận bù" = add-on nếu BA cần. ⚠️ **Pending K**: (b) guard size video minh chứng server 1500 user; (c) low: `_delivery_to_claim` đọc live `request.compensation_unit_qty` (snapshot chưa đóng băng trên allocation — chỉ manager sửa được, đã chặn portal); (d) SO bù để `draft`, HQ tự confirm sinh phiếu giao (không auto-confirm). **Nguồn**: chapter 49 + plan `functional-brewing-quill.md`.
- **S38 — HOTFIX Dashboard Step 1 (2026-07-16, workstream riêng):** gỡ prop `ks_speak` + 12 nút voice-button (TTS của AI đã strip) khỏi tile/kpi/graph → board crash `OwlError: 'ks_speak' is missing` **chỉ ở debug mode** (`?debug=1`; OWL bỏ `validateProps` ở chế độ user thường nên S35 verify lọt). Bump `wj_ks_dashboard_ninja` → 19.0.1.0.1, `-u` RC=0/0 ERROR log, test_sprint9 7/7. **Deploy:** module MỚI trên prod cần `-i` lần đầu (gotcha #15); nếu đã cài thì bump auto-upgrade regen asset. Nguồn sự thật workstream + gotcha #26: `docs/dashboard-migration-plan.md`.
...
14. **Odoo 19 refactor UoM (Sprint K)** — `uom.uom` BỎ `category_id`, chuyển sang CÂY `relative_uom_id` (Reference Unit) + `relative_factor`; `factor` = tích lũy tới gốc cây. ⚠️ `uom._compute_quantity(qty, to_unit, rounding_method='UP')` **KHÔNG kiểm nhóm** (chỉ nhân/chia factor) → quy đổi 2 đơn vị khác nhóm (kg ↔ cái) ra số vô nghĩa, KHÔNG raise. Muốn kiểm cùng nhóm: so **gốc cây** (`while uom.relative_uom_id: uom = uom.relative_uom_id`). Quy đổi 2 chiều (claim↔delivery) cẩn thận **double-UP** (UP(UP(x))>x) → dùng `rounding_method='DOWN'` cho chiều ngược để không vượt. `sale.order.line` ĐVT = `product_uom_id` (không phải `product_uom`). `stock.move` SL đã giao = `move.quantity` (bỏ `quantity_done`); backorder tạo NGAY trong `move._action_done` nên sau `super()._action_done()` đã thấy move backorder pending. Ép `price_unit=0` trên SO line: pass trong create + `.write({'price_unit':0})` sau (giữ 0 nếu SO ở draft, không đổi qty).
