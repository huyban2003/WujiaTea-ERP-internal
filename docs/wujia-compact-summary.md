...
**Cập nhật:** 2026-06-22 — **Sprint 20 DONE (chờ push).** S20 = Mobile Đổi trả/Bù hàng 4 màn (`wujia_portal_return`, Figma `4449:81`, BA mobile_source_ui_v2_3, UI-only) + **fix gốc back-icon kẹt** (Vuexy `.drag-target` z1036 nuốt click → tắt `<768px` ở `_wujia_theme.css` → fix MỌI trang mobile) + tiêu đề nowrap. S18+19 đã push trước đó (`d28bd3b`/`83f98b6`/`7389b6d`/`94f6600`). **▶ NEXT: tiếp UI mobile** (Đổi trả đã xong; còn status batch S13, khung giờ cart S11, i18n). Chi tiết → bảng §4 + §5 + `chapters/`. _(Lịch sử prose sprint cũ đã GỠ để giữ compact.)_
...
**State (2026-06-22) — SPRINT 20 DONE (chờ push); S18+19 đã PUSHED `origin/main`.** 18 module active.
