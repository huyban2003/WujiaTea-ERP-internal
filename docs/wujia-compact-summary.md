...
**Cập nhật:** 2026-07-13 — **Sprint 32 DONE: Controller Thông báo theo BA FINAL + toolchain đọc spec BA (chat GPT + xlsm) — chapter 43.** `wujia_portal_notification` 19.0.1.8.0: rework controller theo bảng mapping BA 7 dòng (chat share `6a54f1b6…` + sheet `1. Model Field`/`3. Controller`). **Giữ model `wujia.notification*`** (map field-name BA: title←name, publish_date←date, category←type_id, mã←dispatch_number). THÊM `expired_date`+`is_expired` (còn-hiệu-lực vs lịch-sử, badge "Đã hết hiệu lực" thay heuristic 30 ngày) + `priority_label`; **priority realign** low/normal/high/urgent → **normal/important/urgent** (migration pre remap low→normal, high→important) nhãn "Lưu ý/Quan trọng/Cần làm" backend trả về; **read theo (noti,user,cửa hàng)** — read model +`franchise_id`+`last_open_date`, unique (noti,user,franchise), badge/unread tách theo store (1 user 2 store đọc độc lập). Controller: `_effective_domain`/`_history_domain` (published + broadcast/target + date≤now [+ not-expired]), filter read_status all/read/unread + priority + date-range (INVALID_DATE_RANGE) + limit clamp {10,20,50} default 10, popup 5 loại hết hạn; **route MỚI** `/mark-all-read` (server tự gom effective-unread, không nhận ids/filter) + `/notification/<id>/attachment/<att>` **guarded — đóng IDOR** (thay link thô `/web/content/ir.attachment`); ERROR_MESSAGES đủ bảng mã BA. Giữ `franchise_ids` targeting (trống=broadcast, thỏa "global"). Perf 1500 user: mọi read/unread = 1 batched query + index kép, không per-record. Verify upgrade RC=0 migration chạy (Odoo xóa selection low/high); test_sprint32 ORM 15/0 + HTTP 15/0 (store-scoped read 29→28, IDOR 403, mark-all→0, expired loại popup còn ở lịch sử); regression sprint9 7/0. **Toolchain MỚI (dev-only, gitignored, KHÔNG lên server): `scripts/ba_spec/{fetch_ba_chat.py,read_xlsm.py}`** — parse chat GPT share (turbo-stream) ra markdown + dump sheet xlsm; skill `/wujia-start` thêm mục "Controller task workflow" (fetch chat + dump 4 sheet + đối chiếu source thật + hỏi ở fork). _(Trước: Sprint 31 lịch sử đơn hàng + realtime cart — chapter 42; Sprint 30 order controller — chapter 41. Prose chi tiết các sprint cũ GỠ khỏi đầu file → xem §4 bảng + chapters/.)_
...
**State (2026-07-13) — SPRINT 32 DONE: Controller Thông báo (BA FINAL) + BA-spec toolchain, chapter 43.** 18 module active.
