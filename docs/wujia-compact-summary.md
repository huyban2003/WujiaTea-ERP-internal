...
**State (2026-07-07) — SPRINT 29 DONE: Mobile Empty State "no-data" chung + Filter canonical `wj-filter-*` (Figma `4788:974` + `4785:498`), 8 module portal mobile, chapter 40.** 18 module active. _(Trước: Sprint 28 PC Tài khoản, chapter 39.)_
- **S29 — Mobile Empty State "no-data" chung + chuẩn hoá Filter (Figma `4788:974` + `4785:498`):** BA giao 2 source mobile (PC = S28 song song, KHÔNG đụng desktop `d-none d-lg-block`). **Task 1** nâng canonical `.wj-empty-state--rich` (`_components.css`) thành card no-data: **icon document 76×76** nhúng SVG data-uri (tải node `4788:1014`, asset đa màu circle `#EAF7FD` + line `#CBD5E1` giữ nguyên) + title 18/700 `#111827` + 2 dòng phụ 14/400 `#6B7280` + nút `.wj-empty-state-btn` cyan 147×40; **dual-mode `:has(>i)`** (span rỗng→document / có glyph→circle-tile 64 giữ icon error semantic). Rollout 7 trang list mobile, giữ copy từng trang, nút **context-aware** (no-data→"Tải lại" reload / lọc rỗng→"Xoá lọc" URL gốc / có CTA→"Tạo ticket/yêu cầu"). **Task 2** dựng `.wj-filter-card` + `.wj-filter-search` (input 38h `#F8FAFC` + btn cyan 38×38) / `.wj-filter-dates` (pill 38h + dash, `--compact` cho biến thể 04) / `.wj-filter-period` (reserve 05); token mới `--wujia-filter-field-bg`. **Chip reconcile** 32/11.3px/700 bọc `@media <992` (desktop PC noti chip 13/600 NGUYÊN) + gỡ **4 override per-page** (morder 30 / mhist 28 / delivery 30). Map: **03** history+delivery (delivery thêm keyword wire `q`), **01** noti+support+knowledge, **02** return (list KHÔNG có `today`→dùng q+date), **04** exam s0 (demo UI-only), **05** reserve công nợ. report/orders + info_request chưa có mobile→ngoài scope. **Dead CSS** search cũ (mknow/mhist/mdelivery/mexam/mreturn) dọn 4 file (grep 0 consumer; giữ `mknow-card`/`icon-danger`). UI-only, giữ `name=` field. manifest 7 module trang + layout `19.0.30.0.0` (đã ở 629b165); `_components.css?v=1149` / `_variables.css?v=1148`. Verify upgrade RC=0 91 mod, Playwright 390px **14 màn + `getComputedStyle` khớp Figma**, 0 class cũ sót, regression desktop chip 13/600 + order-catalog/exam-wizard chip 30→32 sạch, test 7/0+21/0. → chapter 40. ⚠️ **Pending/UI-only**: exam filter = demo (Phase 2 wire); biến thể 05 chưa consumer (chờ công nợ Phase 2); nút "Tải lại" = `location.reload()` đơn giản (chưa AJAX); chip fade-hint mép phải bỏ (clip khi ít chip). **GOTCHA**: (1) base `.wj-empty-state p` (0,1,1) đè `.wj-empty-state-title/sub` (0,1,0)→ra sai màu, fix scope `.wj-empty-state .wj-empty-state-*`; (2) `.wj-filter-chip` dùng CẢ desktop (PC noti) → reconcile bắt buộc bọc `@media <992`; (3) session PC S28 `git add` quét luôn shared file (`assets.xml` ?v + `layout __manifest__` v30) vào commit `629b165` → commit S29 chỉ còn 19 file content mobile.
