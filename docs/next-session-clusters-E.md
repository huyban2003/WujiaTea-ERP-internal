...
| E4a/E4b/E4c | `UI-FILTER-001` (139) | `wujia_portal_layout` + 9 module | ✅ 17–19/09 · E4a nền + E4b1 PC + E4b2 mobile + E4c hành vi lọc, **đóng issue** · xem `docs/f-progress.md` |
| E5a/E5b1/E5b2/E5c | `UI-LISTCARD-001` (136) | `wujia_portal_layout` + 8 module | ✅ 19/09 · component + 11 call site + nhịp/bề ngang 8 khổ, **đóng issue** · xem `docs/f-progress.md` |
| E6a / E6b1 / E6b2 / E6c | `UI-BUTTON-001` (132) | `wujia_portal_layout` + 13 module | **E6a ✅ 20/09** — atom + thước đo `wj_button.py` + 21 call site (Hỗ trợ · Bù hàng · Thông báo), `docs/e6a-acceptance-matrix.md`. **E6b1 ✅ 20/09** — 32 call site (Giao hàng · Thư viện · Yêu cầu thông tin · Lịch sử mua · nền portal · **Công nợ**) + luật ranh giới thanh lọc (E4 FB-08 ↔ E6) + vá bẫy "Pass rỗng" của thước đo, `docs/e6b1-acceptance-matrix.md`; 2 LIMIT: nút icon navbar & `/my/franchises` (khung Odoo gốc) là boundary. **E6b2 ✅ 21/09** — 19 call site (giỏ mobile · panel giỏ PC · chi tiết sản phẩm · **toàn bộ màn auth**) + xoá họ `wj-cta-btn` + thước đo đo được màn chưa đăng nhập, `docs/e6b2-acceptance-matrix.md`; 2 IMPACT (submit auth PC 50→46, lưu mật khẩu mobile 46→48), boundary thêm-vào-giỏ/stepper giữ nguyên. · **E6c ✅ 23/09** — 30 call site màn Thi (21 `wj-btn` + 7 `wj-iconbtn` XML + 2 dựng bằng JS) · ô lịch/khung giờ/FAB là boundary **đọc token `--wj-btn-*`** · gửi đăng ký dùng `.is-loading` · gạch 12 (nhãn dài VI/EN/ZH, submit lặp, bàn phím) · vá tràn ngang màn đăng nhập (gốc: tên ngôn ngữ trong pill 72px) · guard mới *class đi kèm atom không giành dáng*, `docs/e6c-acceptance-matrix.md`. **ĐÃ DEPLOY UAT 24/09**, retest chỉ-đọc 30 màn × 5 khổ 0 vi phạm, `qa_sync` → **Ready for Retest** |
| E7a / E7b | `UI-PAGECONTAINER-001` (129) | `wujia_portal_layout` + mọi module portal | **E7a ✅ 24/09** — `<main.wj-page-container>` trong `app_layout` giữ lề duy nhất (24 PC/tablet · 16 mobile · 12 màn danh sách) · tước lề 3 họ vỏ + Công nợ/Home/Khảo sát · gỡ rule 16px rò của Khảo sát (gốc tiêu đề lệch 16px) · thước đo `wj_pagecontainer.py`, 707 → 0 vi phạm, `docs/e7a-acceptance-matrix.md`. **E7b ✅ code 25/09** — `pc_width` standard 1440/narrow 960 trên chính container (29 template theo mapping BA) · `pc_bottom=sticky` thay đáy chồng của Đặt hàng/giỏ · PC-8 WIDTH, 31 route × 7 khổ 0 vi phạm, 0 ô mật độ giảm, `docs/e7b-acceptance-matrix.md`. **UAT 25/09: 217 ô 0 vi phạm, Khảo sát chi tiết 0 ⇒ Ready for Retest** |
