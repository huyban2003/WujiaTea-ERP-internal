...
\begin{lstlisting}[style=shell]
1. Upgrade portal_layout local:
   bash scripts/upgrade.sh wujia_portal_layout

2. Mở http://localhost:8019/portal, hard-refresh Ctrl+Shift+R:
   - Sidebar menu hiện đủ items, không có overlay xám.
   - 4 card dashboard: header bo tròn 20px khớp với viền card, không lòi.

3. Devtools toggle mobile (≤ 1199px): overlay phủ full width khi mở
   search dropdown (giữ behavior cũ trên mobile).

4. Push lên GitHub → auto-deploy → mở server URL, F5 cache-bust:
   - Sidebar không còn bị overlay.
\end{lstlisting}
