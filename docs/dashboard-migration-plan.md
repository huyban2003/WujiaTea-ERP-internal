...
|---|---|---|---|---|
| 1 | Skill + state file + scaffold/rename/strip AI/port core 18→19 | 35 | **✅ DONE 2026-07-16** (+HOTFIX ks_speak gotcha #26) | install RC=0 + 0 ERROR log; browser render tile + amCharts bar + list (Playwright); 0 JS console error; test_sprint9 7/7. ⚠️ verify browser PHẢI bật `?debug=1` VÀ mở board-detail CÓ item (gotcha #26) |
...
*Danh sách gộp — tất cả đều còn áp cho Step 2b/3. #1–15 = discovery; #16–22 phát hiện khi làm
Step 1; #23–25 khi scaffold Step 2a; #26 khi test lại Step 1 hậu-DONE.*
...
16. **RC=0 NÓI DỐI** *(Step 1, 2026-07-16)*: `odoo-bin -i/-u --stop-after-init` với `logfile=` trong config exit 0 KỂ CẢ khi "Failed to initialize database"/CRITICAL. Verify gate = `: > logs/odoo.log` trước chạy + `grep -cE 'ERROR|CRITICAL'` sau chạy, KHÔNG tin RC.
...
26. *(test Step 1 hậu-DONE, 2026-07-16 — áp cả verify gate)* **AI-strip SÓT `ks_speak` → crash board CHỈ Ở DEBUG MODE.** 3 component `Ksdashboardtile`/`Ksdashboardkpiview`/`Ksdashboardgraph` còn khai prop `ks_speak:{type:Function , Optional:true}` + nút `<div class="voice-button" t-on-click="props.ks_speak">…<audio t-ref="aiAudioRef"/></div>` (TTS của AI đã strip). **`Optional` viết HOA KHÔNG phải key OWL** (OWL chỉ hiểu `optional` thường) ⇒ prop bị coi **bắt buộc**. v18 gốc luôn truyền `ks_speak` từ template AI nên không lộ; bản wj strip AI → grid `ks_dashboard_item_template` không truyền → **`OwlError: 'ks_speak' is missing`**, sập cả board. **Chỉ nổ khi OWL dev mode BẬT** (`env.debug || session.test_mode`, `web/static/src/env.js:236`) — tức browser Developer Mode / `?debug=1`/`?debug=assets`; chế độ user thường OWL **bỏ qua validateProps** (`owl.js:5973` gate `this.dev`) → render OK ⇒ Step 1 verify "0 console error" ở chế độ thường **lọt bug này**. **FIX (2026-07-16):** xoá 3 khai prop `ks_speak` + 12 block voice-button (tile 7, kpi 3, graph 1, list 1); `aiAudioRef` JS refs còn nhưng null-safe (`?.`) — vô hại; `.ks_speaker` CSS + 1 dòng XML comment (list:288) để lại (dead, harmless). Regen asset `-u wj_ks_dashboard_ninja` RC=0 / 0 ERROR log, test_sprint9 7/7. **⇒ Bài học: (a) verify gate browser PHẢI bật `?debug=1` VÀ mở board-detail CÓ item, không chỉ landing; (b) `Optional`/`Required` sai hoa/thường = latent bug toàn Ksolves — grep khi port advance/formula (component AI khác nếu còn).**
