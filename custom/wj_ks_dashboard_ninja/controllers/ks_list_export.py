        ...
        for dataset in list_data['data_rows']:
            ...
            for ks_count, val in enumerate(dataset['data']):
                if isinstance(val, (float, int)):
                    if val >= 0:
                        try:
                            ks_precision = item.sudo().env.ref('wj_ks_dashboard_ninja.ks_dashboard_ninja_precision').digits
                        except Exception as e:
                            ks_precision = 2
...
class KsListExcelExport(KsListExport, http.Controller):
    ...
    @http.route('/wj_ks_dashboard_ninja/export/list_xls', type='http', auth="user")
    def index(self, data):
        try:
...
class KsListCsvExport(KsListExport, http.Controller):
    ...
    @http.route('/wj_ks_dashboard_ninja/export/list_csv', type='http', auth="user")
    def index(self, data):
        try:
            return self.base(data)
        except Exception as exc:
