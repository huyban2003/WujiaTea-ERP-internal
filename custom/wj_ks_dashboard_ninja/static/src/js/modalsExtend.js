...
patch(ConfirmationDialog.prototype,{
    setup(){
        ...
        onMounted( () => {
            ...
            if(modalBody && (this.env.services.menu?.getCurrentApp()?.xmlid === "wj_ks_dashboard_ninja.board_menu_root" ||
                                                    this.env.services.action?.currentController?.action?.tag === 'wj_ks_dashboard_ninja')){
                modalBody.innerHTML = renderToString('wj_ks_dashboard_ninja.ksConfirmationDialogBody', {
...
patch(WarningDialog.prototype,{
    setup(){
        ...
        onMounted( () => {
            ...
            if(modalBody && (this.env.services.menu?.getCurrentApp()?.xmlid === "wj_ks_dashboard_ninja.board_menu_root" ||
                                                    this.env.services.action?.currentController?.action?.tag === 'wj_ks_dashboard_ninja')){
                modalBody.innerHTML = renderToString('wj_ks_dashboard_ninja.ksAccessErrorDialog', {
