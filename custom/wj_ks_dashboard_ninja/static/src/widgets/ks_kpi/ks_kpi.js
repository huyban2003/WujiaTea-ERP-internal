    ...
    ks_changes(){
        ...
        if (field.ks_kpi_data){
            ...
            if (field.ks_goal_enable) {
                ...
                var acheive = diffrence >= 0 ? true : false;
                if (acheive) {
                    this.ks_kpi_ref.el?.querySelectorAll(".target_deviation").forEach((element) => {
            ...
            if (this.ks_kpi_ref.el && this.ks_kpi_ref.el?.querySelectorAll('.row').children?.length !== 2) {
                var filter_children = this.ks_kpi_ref.el?.querySelectorAll('.row').children;
                filter_children?.forEach(function(child) {
            ...
            if (field.ks_previous_period && ks_valid_date_selection.indexOf(field.ks_date_filter_selection) >= 0) {
                ...
                var pre_acheive = pre_diffrence > 0 ? true : false;
                if (pre_acheive) {
                    this.ks_kpi_ref.el?.querySelectorAll(".pre_deviation").forEach((element) => {
                        element.style.color = "green";
                    });
                } else {
                    this.ks_kpi_ref.el?.querySelectorAll(".pre_deviation").forEach((element) => {
            ...
            if (field.ks_target_view === "Progress Bar" && field.ks_goal_enable && field.ks_standard_goal_value && field.ks_model_id_2) {
                this.ks_kpi_ref.el.querySelector('#ks_progressbar').value = parseInt(target_progress_deviation);
            }
