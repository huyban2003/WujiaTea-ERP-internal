...
import { onAudioEnded } from '@wj_ks_dashboard_ninja/js/ks_global_functions';
    ...
    setup(){
        ...
        onWillUpdateProps(async(nextprops)=>{
            if(nextprops.ksdatefilter !='none'){
                await this.ksFetchUpdateItem(this.item.id, this.props.dashboard_data.context)
            }
        ...
        useEffect(()=>{
            if (update_interval && !this.env.inDialog){
                const interval = setInterval(() => {
                    this.ksFetchUpdateItem(this.item.id, this.props.dashboard_data.context);
                }, update_interval);
        ...
        onMounted(()=>{
            if (this.ks_ai_analysis){
                const dashboardItems = this.ks_tile.el.querySelectorAll('.ks_dashboarditem_id');
     ...
     ksFetchUpdateItem(item_id, domain) {
            var self = this;
            if(!domain){
