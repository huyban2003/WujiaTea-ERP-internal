...
import json
from odoo.addons.wj_ks_dashboard_ninja.common_lib.ks_date_filter_selections import ks_get_date, ks_convert_into_local, \
    ks_convert_into_utc
...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_dashboard_top_menu_id = fields.Many2one('ir.ui.menu',
                                               ...
                                               default=lambda self: self.env.ref('wj_ks_dashboard_ninja.dashboards_menu_root',False))
    ...
    ks_dashboard_default_template = fields.Many2one('ks_dashboard_ninja.board_template',
                                                    default=lambda self: self.env.ref('wj_ks_dashboard_ninja.ks_blank',
                                                                                      False),
    ...
    def _default_dashboard_image(self):
        return image_data_uri(encodebytes(file_open('wj_ks_dashboard_ninja/static/images/dashboardOverview/defaultDashboard.png', 'rb').read()))
    ...
    def write(self, vals):
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                if self.env.ref('wj_ks_dashboard_ninja.ks_my_default_dashboard_board') and self.env.ref(
                        'wj_ks_dashboard_ninja.ks_my_default_dashboard_board').sudo().id == rec.id:
                    if self.env.ref('wj_ks_dashboard_ninja.board_menu_root', False):
                        self.env.ref('wj_ks_dashboard_ninja.board_menu_root').sudo().name = vals['ks_dashboard_menu_name']
                else:
                    rec.ks_dashboard_menu_id.sudo().name = vals['ks_dashboard_menu_name']
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('wj_ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    if self.env.ref('wj_ks_dashboard_ninja.board_menu_root', False):
                        self.env.ref('wj_ks_dashboard_ninja.board_menu_root').group_ids = vals['ks_dashboard_group_access']
                else:
    ...
    def unlink(self):
        if self.env.ref('wj_ks_dashboard_ninja.ks_my_default_dashboard_board').id in self.ids:
    ...
    def ks_update_menu_id_old_db(self):
        ...
        ks_menu_id = self.env.ref('wj_ks_dashboard_ninja.dashboards_menu_root').id
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        ks_dn_active_ids = []
        if self.env.context.get('ks_dn_active_ids'):
            ks_dn_active_ids = self.env.context.get('ks_dn_active_ids')
        ...
        has_group_ks_dashboard_manager = self.env.user.has_group('wj_ks_dashboard_ninja.ks_dashboard_ninja_group_manager')
        ks_dashboard_rec = self.browse(ks_dashboard_id)
        zooming_enabled = self.env['ir.config_parameter'].sudo().get_param('wj_ks_dashboard_ninja.enable_chart_zoom')
        dashboard_data = {
            'name': ks_dashboard_rec.name,
            'multi_layouts': ks_dashboard_rec.multi_layouts,
            'ks_company_id': (self.env.context.get('allowed_company_ids') or [self.env.company.id])[0],
            'ks_dashboard_manager': has_group_ks_dashboard_manager,
            'ks_dashboard_list': self.search_read([], ['id', 'name']),
            'ks_dashboard_start_date': self.env.context.get('ksDateFilterStartDate', False) or self.browse(
                ks_dashboard_id).ks_dashboard_start_date,
            'ks_dashboard_end_date': self.env.context.get('ksDateFilterEndDate', False) or self.browse(
                ks_dashboard_id).ks_dashboard_end_date,
            'ks_date_filter_selection': self.env.context.get('ksDateFilterSelection', False) or self.browse(
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        """
        try:
            ks_precision = self.sudo().env.ref('wj_ks_dashboard_ninja.ks_dashboard_ninja_precision')
    ...
    def ks_set_date(self, ks_dashboard_id):
        ...
        if self.env.context.get('ksDateFilterSelection', False):
            ...
            if ks_date_filter_selection == 'l_custom':
                ...
                self = self.with_context(
                    ksDateFilterStartDate=dt.datetime.strptime(ks_start_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                                                                   "%Y-%m-%d %H:%M:%S"))
                self = self.with_context(
                    ksDateFilterEndDate=dt.datetime.strptime(ks_end_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                                                                 "%Y-%m-%d %H:%M:%S"))
    ...
    def fetch_dashboard_overview(self, **kwargs):
        ...
        return {
            ...
            'isManager': self.env.user.has_group('wj_ks_dashboard_ninja.ks_dashboard_ninja_group_manager')
        }
    ...
    def ks_view_items_view(self):
        ...
        return {
            ...
            'search_view_id': self.env.ref('wj_ks_dashboard_ninja.ks_item_search_view').id,
            'context': {
    ...
    def ks_open_import(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('wj_ks_dashboard_ninja.ks_import_dashboard_action')
        return action

    def ks_open_setting(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('wj_ks_dashboard_ninja.board_form_tree_action_window')
        # action['res_id'] = self.id
        # action['target'] = 'new'
        # action['context'] = {'form_view_ref':'wj_ks_dashboard_ninja.board_form'}
    ...
    def ks_prepare_import_data_vals(self, data, menu_id):
        vals = {
            ...
            'ks_dashboard_top_menu_id': menu_id.id if menu_id else self.env.ref(
                "wj_ks_dashboard_ninja.board_menu_root").id,
            'ks_dashboard_active': True,
            'ks_gridstack_config': data['ks_gridstack_config'],
            'ks_dashboard_default_template': self.env.ref("wj_ks_dashboard_ninja.ks_blank").id,
