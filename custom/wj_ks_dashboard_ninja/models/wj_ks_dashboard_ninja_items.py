...
from odoo.addons.wj_ks_dashboard_ninja.common_lib.ks_date_filter_selections import ks_get_date, ks_convert_into_utc, \
...
_ks_m2m_read_orig = fields.Many2many.read

KS_ORDERED_M2M_FIELDS = ('ks_list_view_fields', 'ks_list_view_group_fields',
                         'ks_chart_measure_field', 'ks_chart_measure_field_2')


def ks_read(self, records):
    """Odoo 19 port: delegate to standard Many2many.read, then re-apply the user
    drag&drop column order stored in ks_many2many_field_ordering (JSON per field).
    The v16/v18 version re-implemented read() on Odoo 18 internals
    (get_domain_list) and broke every M2M field on Odoo 19."""
    _ks_m2m_read_orig(self, records)
    if self.model_name == 'ks_dashboard_ninja.item' and self.name in KS_ORDERED_M2M_FIELDS:
        for record in records:
            if not record.ks_many2many_field_ordering:
                continue
            order = json.loads(record.ks_many2many_field_ordering).get(self.name)
            if not order:
                continue
            current = record[self.name]._ids
            ordered = sorted(current, key=lambda x: order.index(x) if x in order else len(order))
            self._insert_cache(record, [tuple(ordered)])
...
def ks_time_addition(self, gb, query):
    ...
    tz_convert = field_type == 'datetime' and self.env.context.get('tz') in pytz.all_timezones
    ...
    if temporal:
        ...
        if tz_convert:
            qualified_field = "timezone('%s', timezone('UTC',%s))" % (self.env.context.get('tz', 'UTC'), qualified_field)
...
class KsDashboardNinjaItems(models.Model):
    ...
    def compute_zoom_enabled(self):
        for rec in self:
            rec.zoom_enabled = self.env['ir.config_parameter'].sudo().get_param('wj_ks_dashboard_ninja.enable_chart_zoom')
    ...
    ks_client_action = fields.Many2one('ir.actions.client',
                                       ...
                                       domain="[('name','!=','App Store'),('name','!=','Updates'),('res_model','not ilike','wj_ks_dashboard_ninja.%'),('name','!=','Discuss')]",
                ...
                try:
                    ...
                    with open(fp.name, 'r', encoding='utf-8') as csvfile:
                        ...
                        for row in header_row:
                            ...
                            self.env.cr.execute(del_group_by_field)
    ...
    def check_target(self):
        ...
        menu_record = self.env.ref('wj_ks_dashboard_ninja.board_menu_root')
    ...
    def _ks_compute_precision_digits(self):
        for rec in self:
            try:
                precision_digits = self.sudo().env.ref('wj_ks_dashboard_ninja.ks_dashboard_ninja_precision')
    ...
    # default = lambda self: self.sudo().env.ref('wj_ks_dashboard_ninja.ks_dashboard_ninja_precision')
    ...
    def _compute_display_name(self):
        # Odoo 19: name_get() removed — display_name compute override
        for rec in self:
            rec.display_name = rec.name or rec.ks_model_id.name
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period and rec.ks_year_period != 0 and rec.ks_dashboard_item_type:
                        ...
                        ks_date_domain = ['&', (date_field_name, ">=",
                                                dt.datetime.strftime(selected_start_date,
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)),
                                          (date_field_name, "<=",
                                           dt.datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT))]

                        for p in range(1, abs_year_period + 1):
                            ks_date_domain.insert(0, '|')
                            ks_date_domain.extend(['&', (date_field_name, ">=", dt.datetime.strftime(
                                selected_start_date - relativedelta.relativedelta(years=p) * sign_yp,
                                DEFAULT_SERVER_DATETIME_FORMAT)),
                                                   (date_field_name, "<=", dt.datetime.strftime(
                                                       selected_end_date - relativedelta.relativedelta(years=p)
                                                       * sign_yp, DEFAULT_SERVER_DATETIME_FORMAT))])
                    else:
                        selected_start_date = dt.datetime.strftime(selected_start_date,
                                                                       DEFAULT_SERVER_DATETIME_FORMAT)
                        selected_end_date = dt.datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT)
                        ks_date_domain = [(rec.ks_date_filter_field.name, ">=", selected_start_date),
                                          (rec.ks_date_filter_field.name, "<=", selected_end_date)]
                elif selected_start_date and not selected_end_date:
                    selected_start_date = dt.datetime.strftime(selected_start_date, DEFAULT_SERVER_DATETIME_FORMAT)
                    ks_date_domain = [(rec.ks_date_filter_field.name, ">=", selected_start_date)]
    ...
    def ks_fetch_list_view_data(self, rec, ks_chart_domain, limit=15, offset=0, ks_export_all=False, initial_count=0):
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                if type(res[field_rec]) == dt.datetime or type(res[field_rec]) == dt.date:
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period_2 and rec.ks_year_period_2 != 0:
                        ...
                        if rec.ks_date_filter_field_2:
                            selected_start_date = dt.datetime.strftime(selected_start_date,
                                                                           DEFAULT_SERVER_DATETIME_FORMAT)
                            selected_end_date = dt.datetime.strftime(selected_end_date,
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)
