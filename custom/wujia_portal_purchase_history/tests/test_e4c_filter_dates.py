"""E4c — Lịch sử mua hàng: khoảng ngày ngược phải nói ra, không im lặng trả rỗng.
...
class TestHistoryFilterDatesE4c(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.franchise = env['wujia.franchise.management'].create({
            'code': 'E4CH', 'name': 'E4CH store',
            'franchise_end_date': '2030-01-01',
            'partner_id': env['res.partner'].create({'name': 'E4CH partner'}).id,
        })
        cls.user = env['res.users'].create({
            'name': 'e4c_hist', 'login': 'e4c_hist', 'password': 'e4c_hist',
    ...
    def _get(self, url):
        self.authenticate('e4c_hist', 'e4c_hist')
    ...
    def test_ngay_nguoc_bao_ngay_tai_thanh_loc(self):
        ...
        html = self._get('/portal/purchase-history' + REVERSED)
        self.assertIn(ERR_DATE_RANGE, html)
        tag = self._tag(html, 'wj-hist-pcerr')
        self.assertTrue(tag, 'thiếu ô báo lỗi wj-hist-pcerr')
    ...
    def test_khong_loi_thi_o_bao_van_ton_tai_nhung_an(self):
        ...
        html = self._get('/portal/purchase-history')
        self.assertNotIn(ERR_DATE_RANGE, html)
        for eid in ('wj-hist-pcerr', 'wj-hist-merr'):
