...
class KsDashboardNinjaItemAdvance(models.Model):
    ...
    listview_redirect_link=fields.Char("Listview Redirect Link")
    ks_redirect_link=fields.Char("Redirect Link")
        ...
        for rec in self:
            ...
            if rec.ks_data_calculation_type == 'query' and rec.ks_dashboard_item_type not in ['ks_tile', 'ks_kpi'] \
                ...
                if rec.ks_dashboard_item_type == 'ks_list_view':
                    ...
                    try:
                        type_code = []
                        new_env = self.env.registry.cursor()
        ...
        if self._context.get('ksDateFilterSelection', False):
            ...
            if ks_date_filter_selection == 'l_custom':
                self = self.with_context(
                    ksDateFilterStartDate=datetime.strptime(self._context['ksDateFilterStartDate'],
                                                                   "%Y-%m-%d %H:%M:%S"))
                self = self.with_context(
                    ksDateFilterEndDate=datetime.strptime(self._context['ksDateFilterEndDate'],
                                                                 "%Y-%m-%d %H:%M:%S"))
