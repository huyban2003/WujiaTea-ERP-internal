...
class RedirectLabels extends CharField {
    setup() {
        super.setup();
        const self = this;
    ...
    _Ks_render_edit() {
        ...
        if (field.ks_query_result && field.ks_data_calculation_type === 'query') {
            ...
            if (ks_query_result.header.length) {
                ...
                if (this.props.record.data.listview_redirect_link == "") {
                    $view.val(false)
                } else {
                    $view.val(this.props.record.data.listview_redirect_link)
                }
...
registry.category("fields").add('redirect_labels', RedirectLabels);