...
class TestLegacyRedirectExam(HttpCase):

    def test_legacy_slug_redirects_permanently(self):
        res = self.url_open('/portal/exam-registration', allow_redirects=False)
        self.assertEqual(res.status_code, 301)
        self.assertEqual(urlsplit(res.headers['Location']).path, '/portal/exam')
