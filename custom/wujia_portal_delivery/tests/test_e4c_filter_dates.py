"""E4c — Giao hàng: khoảng ngày ngược phải nói ra, không im lặng trả rỗng.
...
class TestDeliveryFilterDatesE4c(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.franchise = env['wujia.franchise.management'].create({
            'code': 'E4CD', 'name': 'E4CD store',
            'franchise_end_date': '2030-01-01',
            'partner_id': env['res.partner'].create({'name': 'E4CD partner'}).id,
        })
        cls.user = env['res.users'].create({
            'name': 'e4c_dlv', 'login': 'e4c_dlv', 'password': 'e4c_dlv',
    ...
    def _get(self, url):
        self.authenticate('e4c_dlv', 'e4c_dlv')
    ...
    def test_ngay_nguoc_bao_ngay_tai_thanh_loc(self):
        ...
        html = self._get('/portal/delivery' + REVERSED)
        self.assertIn(ERR_DATE_RANGE, html)
        tag = self._tag(html, 'wj-dlv-pcerr')
        self.assertTrue(tag, 'thiếu ô báo lỗi wj-dlv-pcerr')
    ...
    def test_khong_loi_thi_o_bao_van_ton_tai_nhung_an(self):
        ...
        html = self._get('/portal/delivery')
        self.assertNotIn(ERR_DATE_RANGE, html)
        for eid in ('wj-dlv-pcerr', 'wj-dlv-merr'):
